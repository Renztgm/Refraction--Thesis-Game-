...
# =========================
# CONFIG - ADJUSTED FOR GRID SIZE 5
# =========================
@export var move_speed: float = 4.5  # Increased from 3.0 for larger grid
...
@export var stop_distance: float = 6.0  # Increased from 2.0 - stop when this close to player
@export var waypoint_reach_distance: float = 3  # Increased from 1.0 - half of grid size
...
var can_move: bool = true   # NPC movement flag

@onready var interact_label: Label3D = $"../UI/InteractLabel"
...
@onready var bookmark = get_tree().get_first_node_in_group("bookmark")
...
# HELPER FUNCTIONS
# =========================
func find_nearest_walkable_cell(center: Vector2, max_radius: int = 5) -> Vector2:
	# Search in expanding circles around the center cell
	for radius in range(1, max_radius + 1):
		for x in range(-radius, radius + 1):
			for y in range(-radius, radius + 1):
				# Check cells on the perimeter of current radius
				if abs(x) == radius or abs(y) == radius:
					var check_cell = center + Vector2(x, y)
					if grid_system.is_walkable(check_cell):
						return check_cell
	
	return Vector2.ZERO  # No walkable cell found
...
func _ready():
	...
	if not is_instance_valid(bookmark):
		push_error("bookmark not found! Check the path.")
	last_position = global_position

func _physics_process(delta):
	if not is_instance_valid(grid_system) or not is_instance_valid(bookmark):
		return
...
func update_path():
	var distance_to_player = global_position.distance_to(bookmark.global_position)
	...
	var end_cell = grid_system.world_to_grid(bookmark.global_position)
	
	# FIX: Find nearest walkable cell if NPC is on non-walkable cell
	if not grid_system.is_walkable(start_cell):
		print("⚠️ NPC on non-walkable cell, finding nearest walkable cell...")
		var nearest = find_nearest_walkable_cell(start_cell)
		if nearest != Vector2.ZERO:
			start_cell = nearest
			print("✅ Found walkable cell at: ", start_cell)
		else:
			print("❌ No walkable cell found nearby!")
			return
	
	# FIX: Find nearest walkable cell if bookmark is on non-walkable cell
	if not grid_system.is_walkable(end_cell):
		print("⚠️ Bookmark on non-walkable cell, finding nearest walkable cell...")
		var nearest = find_nearest_walkable_cell(end_cell)
		if nearest != Vector2.ZERO:
			end_cell = nearest
			print("✅ Found walkable cell at: ", end_cell)
		else:
			print("❌ No walkable cell found nearby!")
			return
	
	# DEBUG: Print grid cells and walkability
	print("🔍 DEBUG PATHFINDING:")
	print("  NPC World Pos: ", global_position)
	print("  Bookmark World Pos: ", bookmark.global_position)
	print("  NPC Grid Cell: ", start_cell)
	print("  Bookmark Grid Cell: ", end_cell)
	...
	if new_path.size() > 0:
		...
		print("✅ Path found with ", new_path.size(), " waypoints")
		...
		print("⚠️ No path found from NPC to bookmark")
		print("  Possible reasons:")
		print("    - Start or end cell is not walkable")
		print("    - Cells are completely blocked by obstacles")
		print("    - Grid system not properly configured")
		current_path.clear()
		if is_instance_valid(grid_system):
			grid_system.clear_path_visualization()
...
func follow_path(delta):
	...
	# DEBUG: Uncomment to see pathfinding info
	# print("NPC pos: ", global_position, " Target: ", target_pos, " Distance: ", distance_to_waypoint)
